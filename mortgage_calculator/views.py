...
class AffordabilityCalculatorView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        interest_rate = (
            float(request.POST.get("interest_rate")) / 100 / 12
        )  # Monthly rate
        ...
        max_loan = (
            max_affordable_payment * ((1 + interest_rate) ** mortgage_term - 1)
        ) / (interest_rate * (1 + interest_rate) ** mortgage_term)
        monthly_payment = (
            max_loan * interest_rate * (1 + interest_rate) ** mortgage_term
...
class OverpaymentCalculatorView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        monthly_repayment = (
            float(request.POST.get("monthly_repayment"))
            if request.POST.get("monthly_repayment")
            else None
        )
        one_off_overpayment = (
            float(request.POST.get("one_off_overpayment"))
            if request.POST.get("one_off_overpayment")
            else 0
        )
        recurring_overpayment = (
            float(request.POST.get("recurring_overpayment"))
            if request.POST.get("recurring_overpayment")
            else 0
        )
        ...
        if interest_only:
            ...
            if monthly_repayment:
                ...
                monthly_payment = (
                    cost_of_debt
                    * interest_rate
                    * (1 + interest_rate) ** (mortgage_term * 12)
                ) / ((1 + interest_rate) ** (mortgage_term * 12) - 1)
