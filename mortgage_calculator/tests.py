...
class MortgageCalculatorTests(TestCase):
    ...
    def test_basic_mortgage_calculation(self):
        ...
        self.assertAlmostEqual(
            schedule[-1]["ending_balance"], 0, delta=1
        )  # Use delta to check for near 0. Floating point calculations can have some small variations.
    ...
    def test_combined_overpayments(self):
        ...
        self.assertAlmostEqual(
            schedule[2]["remaining_balance"], 190500, delta=100
        )  # Use delta to check for near 0. Floating point calculations can have some small variations.
        self.assertEqual(schedule[2]["total_paid_standard"], 44826)
        self.assertAlmostEqual(
            schedule[2]["total_paid_overpayment"], 45400, delta=100
        )  # Use delta to check for near 0. Floating point calculations can have some small variations.
